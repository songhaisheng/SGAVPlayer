...
@objc protocol SGAVPlayerViewDelegate
{
    func bbAVPlayerViewDidPlayerFailed()
    func bbAVPlayerViewDidFullScreen(isFullScreen: Bool)
}

class SGAVPlayerView: UIView {
    ...
    // MARK: - properties
    weak var delegate: SGAVPlayerViewDelegate?
    
    fileprivate var playerState: SGAVPlayerState = .finished {
        didSet {
    ...
    fileprivate var isSlideSliding: Bool = false
    ...
    fileprivate var sourcePlayControlViewFrame: CGRect = .zero
        ...
        if let playerItem = object as? AVPlayerItem, let key = keyPath
        {
            if key == "status"
            {
                ...
                case .failed:
                    ...
                    guard let _ = self.delegate?.bbAVPlayerViewDidPlayerFailed() else {
                        return
                    }
                default:
                    break
            ...
            {
                let timeInterval: Double = self.availabelDuration()
    ...
    internal func eventFullScreenButtonClicked(_ sender: UIButton)
    {
        ...
        if self.isFullScreen
        {
            ...
            self.controlView.updateViews(orientation: UIDeviceOrientation.portrait)
    ...
    // 播放视频结束通知
    internal func enentVideoPlayDidEndNotification(_ notification: Notification)
    {
    ...
    // MARK: - UISlider控制回调(TouckBegan, ValueChanged, TouchEnded)
    internal func eventSliderTouchBegan(sender: UISlider)
    {
        self.isSlideSliding = true
        self.loadingView?.startAnimating()
    }
    
    internal func enentSliderTouchEnded(sender: UISlider)
    {
        self.isSlideSliding = false
    ...
    internal func eventDeviceOrientationChanged()
    {
        if self.isFullScreen
        {
            self.videoPlayerLayer?.backgroundColor = UIColor.blue.cgColor
            let orientation: UIDeviceOrientation = UIDevice.current.orientation
            switch orientation {
            case .portrait, .portraitUpsideDown:
                self.defultViewOrientation()
            case .landscapeLeft:
    ...
    fileprivate func initViews()
    {
        ...
        self.sourcePlayControlViewFrame = self.controlView.frame
    ...
    fileprivate func initPlayerItem() -> Self
    {
        if let path = self.urlPath, let encodeUrlPath = path.urlQueryEncodingAllowed(), let url = URL(string: encodeUrlPath)
        {
            ...
            self.videoPlayerItem?.addObserver(self, forKeyPath: "playbackLikeToKeepUp", options: NSKeyValueObservingOptions.new, context: nil)
        }
        return self
    }
    ...
    fileprivate func syncprocessSlider()
    {
        ...
        if self.totalDuration.isFinite
        {
            ...
            self.controlView.lbLeftTime.text = Int(self.currentDuration).formatterSeconds()
        }
    ...
    fileprivate func readyToPlayer()
    {
        ...
        self.controlView.lbRightTime.text = Int(self.totalDuration).formatterSeconds()
        self.playerState = .readuToPlay
        self.layer.addSublayer(self.videoPlayerLayer!)
        self.controlView.alpha = 1.0
        self.bringSubview(toFront: self.controlView)
        self.controlView.btnPlay.setImage(UIImage(named: "control_pause"), for: UIControlState.normal)
    ...
    fileprivate func playOrPause(_ sender: UIButton?)
    {
        if self.videoPlayer?.rate != 1.0
        {
            ...
            {
                button.setImage(UIImage(named: "control_pause"), for: UIControlState.normal)
            }
            self.videoPlayer?.play()
        } else
        {
            if let button = sender
            {
                button.setImage(UIImage(named: "control_play"), for: UIControlState.normal)
            }
            self.videoPlayer?.pause()
            self.playerState = .paused
        }
    ...
    /**
     ...
     - returns: 缓冲进度
     */
    fileprivate func availabelDuration() -> Double
    {
    ...
    fileprivate func defultViewOrientation()
    {
    ...
    {
        self.controlView.btnFullScreen.setImage(UIImage(named: "fullscreen"), for: UIControlState.normal)
        self.removeFromSuperview()
        self.backgroundColor = UIColor.clear
        ...
        self.videoPlayerLayer?.backgroundColor = UIColor.clear.cgColor
        self.controlView.transform = CGAffineTransform.identity
        self.controlView.frame = self.sourcePlayControlViewFrame
        self.controlView.updateViews(orientation: UIDeviceOrientation.portrait)
